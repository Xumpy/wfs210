...
import be.velleman.nicowfs210.Channel.InputCoupling;
import be.velleman.nicowfs210.Trigger.ManualTriggering;
import be.velleman.nicowfs210.Trigger.RestartTriggering;
import be.velleman.nicowfs210.Trigger.TriggerChannel;
import be.velleman.nicowfs210.Trigger.TriggerMode;
import be.velleman.nicowfs210.Trigger.TriggerSlope;
...
public class RealWFS210 extends WFS210 implements ConnectionListener {
	private Map<String, String> wifiSettings = new HashMap<String, String>();
        private static final Logger log = Logger.getLogger(RealWFS210.class);
	...
	public Boolean sendSettings() {
		try {
			...
			log.debug("RequestSettings: " + SettingsPacket);
	...
	public Boolean requestSettings() {
		Packet requestSettings = new Packet(8);
                log.debug("Commands.REQUEST_STATUS: " + Commands.REQUEST_STATUS);
                
		requestSettings.setCommand(Commands.REQUEST_STATUS);
		requestSettings.finalize();
                log.debug("RequestSettings: " + requestSettings);
		try {
	...
	public Boolean requestWifiSettings() {
		...
		requestSettings.finalize();
                ...
                log.debug("Send requestWifiSettings: " + requestSettings);
	...
	public void newPacketFound(Packet p) {
		...
		if (currentPacket.getCommand() == Commands.RECEIVE_WIFI_SETTINGS
			...
			log.error("WFS210_WIFISETTINGS - new wifisettings received");
		}
